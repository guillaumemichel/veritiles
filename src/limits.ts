// The SPEC.md §8 limits table as named constants — the single source of the
...
// Format (SPEC §3.1, §4): per-codec block caps and the shared integer ceiling.
...
// Ranges-path bounds (SPEC §4, §8).
export const MAX_LEAF_BYTES = 2 ** 23; // a single verified leaf: 8 MiB

// VerifiedFile client defaults.
export const DEFAULT_MAX_PROOF_BYTES = 32 * 2 ** 20; // unverified proof body (SPEC §8)
...
export const MAX_SEGMENTS = 32; // request path segment count (SPEC §5.4)
export const MAX_NAME = 255; // path / link name UTF-8 bytes (SPEC §4)
